...
class MemberListContainer extends Component {
  ...
  render() {
    ...
    const tableSource = members.map(members => {
      const profileUrl = showMemberLinks ?
        `/members/${members.handle}` : null
      const mailtoURL = `mailto:${members.email}`
      const altTitle = `${members.name} (${members.handle})`
      return Object.assign({}, members, {
        ...
        chapterName: (members.chapter || {}).name,
        phaseNumber: ((members || {}).phase || {}).number,
        email: <Link to={mailtoURL}>{members.email}</Link>,
        active: members.active ? 'Yes' : 'No',
      })
    })
